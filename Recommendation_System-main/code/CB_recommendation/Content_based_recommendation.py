...
import sys
sys.path.append('..')
from measure.measure_method import Similarity
...
def CB_recommend_estimate(user_feature, item_feature_matrix, item):
    ...
    # print(pearsonr(item_feature_matrix.loc[item].values,np.array(item_feature_matrix.loc[user_rated_items, :].values)[0]))
    ...
    similarity = Similarity(item_feature_vector, user_rated_items_matrix)
    # cos_xc = cos_measure(item_feature_vector, user_rated_items_matrix)
    cos_xc = similarity.cos_maesure()
    pe = similarity.pearson_measure()
    jaccard = similarity.jaccard_measure()
    # pe = np.matrix(np.corrcoef(item_feature_vector,user_rated_items_matrix)[0,1:])
    #计算uesr对该item的评分估计
...
def CB_recommend_top_K(user_feature, item_feature_matrix, K):
    ...
    x  = item_feature_matrix.loc[user_unrated_item_index, :].values
    #user喜好总特征
...
if __name__ == '__main__':
    ...
    # CB_recommend_estimate(user_feature, movies_feature, 9)
    # CB_recommend_top_K(user_feature, movies_feature, 10)
    print (CB_recommend_estimate(user_feature, movies_feature, 9))
