...
class LFM(object):
    ...
    def Top_N(self,lfm,uid,N,movies,dataset):
        estimated = dataset[dataset['userId']==uid]['movieId'].tolist()
        tobeestimate = movies[~(movies['movieId'].isin(estimated))]


        for index,row in tobeestimate.iterrows():
            tobeestimate.loc[index,'rate'] = lfm.predict(uid,int(tobeestimate.loc[index,'movieId']))

        tobeestimate = tobeestimate.sort_values(by='rate',ascending=False)
        return tobeestimate['movieId'][:N].tolist()
...
if __name__ == '__main__':
    ...
    tobeestimate = tobeestimate.sort_values(by='rate',ascending=False)
    print('推荐的电影编号')
    print(lfm.Top_N(uid,N,movies,dataset))
