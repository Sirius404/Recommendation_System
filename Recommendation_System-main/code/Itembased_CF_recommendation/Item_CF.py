...
def BuildTarin(startTime):
    ...
    with open('../data/Moivelens/ml-latest-small/ratings.csv') as f:
...
def Recommendation(train,userId,W,K,N):
    ...
    for itemId,score in items.items():
        ...
        for j,wij in sorted(W[itemId],key=lambda x: x[1],reverse=True):
            ...
            rank[j]["weight"] += float(score)*wij


            rank[j]["reason"][itemId]=float(score)*wij
...
if __name__ == "__main__":
    ...
    topN=Recommendation(train,str(1),W,10,5)
    items=[]
    for i in topN:
        items.append(i[0])
